...
-- Map <Leader>z to toggle zoom using tabs
vim.keymap.set("n", "<Leader>z", function()
    -- If there is more than one window in the current tab, we are in a split. Zoom in.
    if #vim.api.nvim_tabpage_list_wins(0) > 1 then
        vim.cmd("tab split")
        -- If there is only one window, we are already zoomed. Close the tab to zoom out.
